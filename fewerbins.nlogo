turtles-own [ trash-on-hand trash-tolerance ]
patches-own [ trash-on-ground ]
...
globals [ total-trash-on-ground ]

to setup
  clear-all
  set-default-shape wastefuls "person custom"
  set-default-shape bins "garbage can"
  ...
  create-bins 2 [ setxy who * 16 29 ]
  create-houses 120 [ setxy who * 6 -15.6 ]
  create-trees 12 [ setxy who * 6 17.5 ]
  ...
  ask wastefuls [
    ...
    set trash-on-hand 0
    set trash-tolerance 0
    update-trash-on-hand
  ]
  ask patches [
    set trash-on-ground 0
    update-patch
  ]
  set-current-plot "Total Trash on Ground"
  set-current-plot-pen "trash"
...
;  ]
  ...
  [
    ...
    random-walk
    if (trash-tolerance > max-trash-tolerance)
    [ set trash-tolerance max-trash-tolerance ]
    if(trash-on-hand = 1)
    [ set trash-tolerance (trash-tolerance + 1) ]
    set label (round trash-tolerance)
  ]

;  update-environment
  ask patches [ update-patch ]
  ask wastefuls [ update-trash-on-hand ]
  plot total-trash-on-ground
  tick
end
...
;  ifelse (trash-on-ground = "new" )
;  [
;    if (energy <= max-stored-energy - 2)
;      [ set energy energy + 2 ]
;  ]
;  [
;    ifelse (trash-on-ground = "recycled" )
;    [
...
to wasteful-process-patch
  if (trash-on-hand = 1)
  [
    ifelse (trash-tolerance = max-trash-tolerance)
    [ ;; can't wait must drop now
      set trash-on-ground trash-on-ground + 1
      set trash-tolerance 0
      set trash-on-hand 0
    ]
    [ ;; look to see where to throw while still can tolerate
      let patches-in-vision  (patches in-cone 4 90)
      if (any? bins-on patches-in-vision)
      [
        let throwing-target one-of patches-in-vision
        face throwing-target
        move-to throwing-target
        ask throwing-target [ set trash-on-ground trash-on-ground + 1 ]
      ]
    ]
  ]
end
...
to update-patch
  set pcolor palette:scale-gradient
        [  [ 34 139 34 ] [ 240 230 140 ] [178 34 34]] trash-on-ground 0 8
  set total-trash-on-ground total-trash-on-ground + trash-on-ground
end

to update-trash-on-hand
  if(trash-on-hand = 0)
  [
    if(random-poisson lambda = lambda) ;; 50 33 25 20%
    [ set trash-on-hand 1 ]
  ]
end
...
;  ask patches with [ trash-on-ground > 0 ]
;  [
...
"" ""
PENS
"trash" 1.0 0 -5298144 true "" "plot total-trash-on-ground"
...
false
0
Polygon -6459832 true false 150 11 30 221 270 221
...
false
11
Polygon -1 false false 60 240 66 257 90 285 134 299 164 299 209 284 234 259 240 240
Rectangle -7500403 true false 60 75 240 240
Polygon -7500403 true false 60 238 66 256 90 283 135 298 165 298 210 283 235 256 240 238
Polygon -7500403 true false 60 75 66 57 90 30 135 15 165 15 210 30 235 57 240 75
Polygon -7500403 true false 60 75 66 93 90 120 135 135 165 135 210 120 235 93 240 75
Polygon -1 false false 59 75 66 57 89 30 134 15 164 15 209 30 234 56 239 75 235 91 209 120 164 135 134 135 89 120 64 90
Line -1 false 210 120 210 285
Line -1 false 90 120 90 285
Line -1 false 125 131 125 296
Line -1 false 65 93 65 258
Line -1 false 175 131 175 296
Line -1 false 235 93 235 258
Polygon -1 false false 112 52 112 66 127 51 162 64 170 87 185 85 192 71 180 54 155 39 127 36
...
false
7
Rectangle -1 true false 45 135 270 255
Rectangle -7500403 true false 124 195 187 256
Rectangle -7500403 true false 60 195 105 240
Rectangle -7500403 true false 60 150 105 180
Rectangle -7500403 true false 210 150 255 180
Line -16777216 false 270 135 270 255
Polygon -6459832 true false 30 135 285 135 240 90 75 90
Line -16777216 false 30 135 285 135
Line -16777216 false 255 105 285 135
Rectangle -7500403 true false 210 195 255 240
Rectangle -7500403 true false 135 150 180 180
...
Rectangle -16777216 true false 123 76 176 92
Circle -1 true false 110 5 80
Line -16777216 false 121 90 194 90
Line -16777216 false 148 143 150 196
Rectangle -16777216 true false 116 186 182 198
Circle -1 true false 152 143 9
Circle -1 true false 152 166 9
Rectangle -16777216 true false 179 164 183 186
Polygon -13791810 true false 180 90 195 90 183 160 180 195 150 195 150 135 180 90
Polygon -13791810 true false 120 90 105 90 114 161 120 195 150 195 150 135 120 90
Rectangle -13791810 true false 120 90 180 135
Polygon -13791810 true false 105 90 90 120 120 135
Polygon -13791810 true false 195 90 210 120 180 135
