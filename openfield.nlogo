...
to setup
  clear-all
  set-default-shape turtles "person custom"
...
to wasteful-process-patch
  ...
  [
    ...
    [ ;; look to see where to throw while still can tolerate
      let patches-in-vision  (patches in-cone 3 90)
      let sum-trash-on-ground (sum [trash-on-ground] of patches-in-vision)
      if (sum-trash-on-ground > trash-area-treshold)
      [ ask one-of patches-in-vision [ set trash-on-ground trash-on-ground + 1 ] ]
    ]
  ]
end
...
to update-patch
  set pcolor palette:scale-gradient
        [  [ 34 139 34 ] [ 240 230 140 ] [255 0 0]] trash-on-ground 0 8
end
...
Rectangle -13345367 true false 120 90 180 135
Polygon -13345367 true false 105 90 90 120 120 135
Polygon -13345367 true false 195 90 210 120 180 135
